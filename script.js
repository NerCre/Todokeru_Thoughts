...
(() => {
  ...
  function extractIdFromSMS(text) {
    if (!text) return '';
    // 1) Prefer explicit "職員ID" line (most reliable for the SMS template)
    //    e.g. "職員ID：S001" / "職員ID:S001"
    const labeled = text.match(/職員ID\s*[：:]\s*([A-Za-z0-9_-]+)/);
    if (labeled && labeled[1]) return labeled[1].trim();

    // 2) Fallback for QR-like payloads that may be pasted into SMS欄
    //    e.g. "STAFF｜S001｜佐藤 一郎" / "STAFF|S001|..."
    const staffBar = text.match(/STAFF[|｜]\s*([A-Za-z0-9_-]+)/i);
    if (staffBar && staffBar[1]) return staffBar[1].trim();

    // 3) Heuristic fallback: split on common separators and pick the first plausible ID
    //    - require at least one ASCII letter to avoid dates/times (e.g. 2026/01/23, 09:16)
    const tokens = text
      .replace(/[\n\r]/g, ' ')
      .replace(/[、，]/g, ' ')
      .replace(/[：:|｜\[\]【】（）()]/g, ' ')
    ...
    const blacklist = new Set(['STAFF', 'PLACE', 'ID', 'SMS', '命をツナゲル', '被災連絡', '連絡時間', '職員ID', '場所', '状態1', '状態2', '事故種別']);
