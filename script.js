...
(() => {
  ...
  const DEFAULT_STAFF = [
    { id: 'S001', name: '佐藤 一郎', affiliation: '自社', birthday: '', blood: '', history: [], meds: [], allergies: [], doctor: '', contactRel: '', contactTel: '' },
    { id: 'S002', name: '高橋 花子', affiliation: '自社', birthday: '', blood: '', history: [], meds: [], allergies: [], doctor: '', contactRel: '', contactTel: '' },
    { id: 'S003', name: '山田 太郎', affiliation: 'A造船', birthday: '', blood: '', history: [], meds: [], allergies: [], doctor: '', contactRel: '', contactTel: '' },
    { id: 'S004', name: '伊藤 次郎', affiliation: 'A造船', birthday: '', blood: '', history: [], meds: [], allergies: [], doctor: '', contactRel: '', contactTel: '' },
    { id: 'S005', name: '鈴木 三郎', affiliation: 'B株式会社', birthday: '', blood: '', history: [], meds: [], allergies: [], doctor: '', contactRel: '', contactTel: '' },
    { id: 'S008', name: '造船 太郎', affiliation: '自社', birthday: '', blood: '', history: [], meds: [], allergies: [], doctor: '', contactRel: '', contactTel: '' },
    { id: 'id-5fd2bb7cd942d8-19bb45d165e', name: '高橋 花子', affiliation: '自社', birthday: '', blood: '', history: [], meds: [], allergies: [], doctor: '', contactRel: '', contactTel: '' },
    { id: 'U006', name: '山田 太郎', affiliation: 'A造船', birthday: '', blood: '', history: [], meds: [], allergies: [], doctor: '', contactRel: '', contactTel: '' },
    { id: 'N009', name: '造船 次郎', affiliation: 'A造船', birthday: '', blood: '', history: [], meds: [], allergies: [], doctor: '', contactRel: '', contactTel: '' },
    { id: 'R010', name: '鈴木 三郎', affiliation: 'B株式会社', birthday: '', blood: '', history: [], meds: [], allergies: [], doctor: '', contactRel: '', contactTel: '' }
  ];
  ...
  function populateNameList(query) {
    ...
    list.innerHTML = '';
    const q = (query || '').trim();
    // filter by kana or name (simple contains)
    ...
    items.forEach((s) => {
      ...
      div.innerHTML = `<span>${escapeHtml(s.name)}</span><span class="sub" style="font-size:12px;color:var(--muted);">${escapeHtml(s.id)}</span>`;
      div.addEventListener('click', () => {
        const modal = document.getElementById('nameModal');
  ...
  async function startQrCamera() {
    // If BarcodeDetector not available, guide user
    const video = document.getElementById('qrVideo');
    const statusEl = document.getElementById('qrStatus');
    ...
    try {
      ...
      if ('BarcodeDetector' in window) {
        ...
        // Without detector: inform user
        if (statusEl) statusEl.textContent = 'QR自動検出が利用できません。写真読み取りか手入力を使ってください。';
      }
    } catch (err) {
  ...
  function onQrRead(value) {
    ...
    closeQrModal();
    if (!value) return;
    // If helmet: value may contain ID and name separated by space or comma
    if (qrPurpose === 'location') {
      if (qrTarget) qrTarget.value = value;
      toast(`場所: ${escapeHtml(value)} を設定しました`);
      return;
    }
    // helmet
    // Attempt to parse ID and name (split by whitespace)
    let id = '';
    let name = '';
    ...
    const msg = name ? `職員ID：${escapeHtml(id)}　氏名：${escapeHtml(name)}\nこちらの職員であっていますか？` : `職員ID：${escapeHtml(id)}\nこちらの職員であっていますか？`;
    // Show confirm modal
    ...
    if (confirmEl && confirmMsg) {
      ...
      okBtn.addEventListener('click', () => {
        ...
        // If a separate name field is provided (edit mode), populate it
      ...
      const ok = window.confirm(msg.replace(/<br>/g, '\n'));
      if (ok && qrTarget) {
        qrTarget.value = id;
        if (qrNameTarget && name) {
          qrNameTarget.value = name;
