...
(() => {
  ...
  function extractIdFromSMS(text) {
    ...
    // 1) Prefer explicit "職員ID:" line if present (most reliable)
    try {
      const lineMatch = t.match(/職員ID\s*[：:]+\s*([^\n\r]+)/);
      if (lineMatch) {
        const after = lineMatch[1] || '';
        // Prefer S + digits (e.g., S001) even if other tokens exist (e.g., STAFF｜S001｜...)
        const sMatch = after.match(/S\d{3,}/i);
        if (sMatch) return sMatch[0].toUpperCase();
        // Fallback: first alphanumeric token
        const idMatch = after.match(/[A-Za-z0-9_-]{2,}/);
        if (idMatch) return idMatch[0];
      }
    } catch (e) {
      console.warn('extractIdFromSMS line parse error', e);
    }

    // 2) Look for common staff-id patterns anywhere in the text
    const sAny = t.match(/S\d{3,}/i);
    if (sAny) return sAny[0].toUpperCase();

    // 3) Fallback: tokenize by whitespace and common separators
    const tokens = t
      .replace(/[\n\r]/g, ' ')
      .replace(/[、，,]/g, ' ')
      .replace(/[：:｜|]/g, ' ')
    ...
    for (const token of tokens) {
      if (/^[A-Za-z0-9_-]{2,}$/.test(token)) return token;
    }
    return '';
  }
    }
  ...
  $('#smsInput').addEventListener('input', (ev) => {
    ...
    if (id) $('#empId').value = id;
  });

  // Some environments may not fire input immediately on paste; ensure paste also triggers extraction.
  $('#smsInput').addEventListener('paste', () => {
    setTimeout(() => {
      const id = extractIdFromSMS($('#smsInput').value);
      if (id) $('#empId').value = id;
    }, 0);
  });
/** =========================
